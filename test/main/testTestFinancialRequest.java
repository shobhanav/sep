package main;
import com.windowtester.runtime.swing.locator.JButtonLocator;
import com.windowtester.runtime.swing.UITestCaseSwing;
import com.windowtester.runtime.IUIContext;
import com.windowtester.runtime.swing.locator.LabeledTextLocator;
import com.windowtester.runtime.swing.locator.JListLocator;
import com.windowtester.runtime.swing.locator.NamedWidgetLocator;

public class testTestFinancialRequest extends UITestCaseSwing {

	/**
	* Create an Instance
	 */
	public testTestFinancialRequest() {
		super(main.Login.class);
	}

	/**
	* Main test method.
	*/
	public void testTestFinancialRequest() throws Exception {
		IUIContext ui = getUI();
		ui.click(new JButtonLocator("Login"));
		ui.click(new JButtonLocator("REP"));
		ui.click(new JButtonLocator("Create"));
		ui.click(new JButtonLocator("Create"));
		ui.click(new JButtonLocator("Create"));
		ui.click(new JButtonLocator("Create"));
		ui.click(new JButtonLocator("Sign out"));
		ui.click(new LabeledTextLocator("Username:"));
		ui.enterText("scso");
		ui.click(new LabeledTextLocator("Password:"));
		ui.enterText("password");
		ui.click(new JButtonLocator("Login"));
		ui.click(new JButtonLocator("REP"));
		ui.click(new JListLocator("Id: 0, Client: XYZ corp, createdBy: cso1, Status: CREATED"));
		ui.click(new JButtonLocator("Send to Fm"));
		ui.click(new JListLocator("Id: 1, Client: XYZ corp, createdBy: cso1, Status: CREATED"));
		ui.click(new JButtonLocator("Send to Fm"));
		ui.click(new JListLocator("Id: 2, Client: XYZ corp, createdBy: cso1, Status: CREATED"));
		ui.click(new JButtonLocator("Send to Fm"));
		ui.click(new JListLocator("Id: 3, Client: XYZ corp, createdBy: cso1, Status: CREATED"));
		ui.click(new JButtonLocator("Send to Fm"));
		ui.click(new JButtonLocator("Sign out"));
		ui.click(new LabeledTextLocator("Username:"));
		ui.enterText("fm");
		ui.click(new LabeledTextLocator("Password:"));
		ui.enterText("password");
		ui.click(new JButtonLocator("Login"));
		ui.click(new JButtonLocator("REP"));
		ui.click(new JListLocator("Id: 0, Client: XYZ corp, createdBy: cso1, Status: REVIEWED_BY_SCSO"));
		ui.click(new JButtonLocator("Send to Admin"));
		ui.click(new JListLocator("Id: 1, Client: XYZ corp, createdBy: cso1, Status: REVIEWED_BY_SCSO"));
		ui.click(new JButtonLocator("Send to Admin"));
		ui.click(new JListLocator("Id: 2, Client: XYZ corp, createdBy: cso1, Status: REVIEWED_BY_SCSO"));
		ui.click(new JButtonLocator("Send to Admin"));
		ui.click(new JListLocator("Id: 3, Client: XYZ corp, createdBy: cso1, Status: REVIEWED_BY_SCSO"));
		ui.click(new JButtonLocator("Send to Admin"));
		ui.click(new JButtonLocator("Sign out"));
		ui.click(new LabeledTextLocator("Username:"));
		ui.enterText("admin");
		ui.click(new LabeledTextLocator("Password:"));
		ui.enterText("password");
		ui.click(new JButtonLocator("Login"));
		ui.click(new JButtonLocator("REP"));
		ui.click(new JListLocator("Id: 0, Client: XYZ corp, createdBy: cso1, Status: REVIEWED_BY_FM"));
		ui.click(new JButtonLocator("Approve"));
		ui.click(new JListLocator("Id: 1, Client: XYZ corp, createdBy: cso1, Status: REVIEWED_BY_FM"));
		ui.click(new JButtonLocator("Approve"));
		ui.click(new JListLocator("Id: 2, Client: XYZ corp, createdBy: cso1, Status: REVIEWED_BY_FM"));
		ui.click(new JButtonLocator("Approve"));
		ui.click(new JListLocator("Id: 3, Client: XYZ corp, createdBy: cso1, Status: REVIEWED_BY_FM"));
		ui.click(new JButtonLocator("Approve"));
		ui.click(new JButtonLocator("Sign out"));
		ui.click(new LabeledTextLocator("Username:"));
		ui.enterText("pm");
		ui.click(new LabeledTextLocator("Password:"));
		ui.enterText("password");
		ui.click(new JButtonLocator("Login"));
		ui.click(new JButtonLocator("Crd"));
		ui.click(new JButtonLocator("previous "));
		ui.click(new JButtonLocator("Finance"));
		ui.click(new JButtonLocator("previous "));
		ui.click(new JButtonLocator("Recruitment"));
		ui.click(new JButtonLocator("previous "));
		ui.click(new JButtonLocator("Task"));
		ui.click(new JButtonLocator("previous "));
		ui.click(new JButtonLocator("SubTask"));
		ui.click(new JButtonLocator("previous "));
		ui.click(new JButtonLocator("signout"));
		ui.click(new LabeledTextLocator("Username:"));
		ui.enterText("scso");
		ui.click(new LabeledTextLocator("Password:"));
		ui.enterText("password");
		ui.click(new JButtonLocator("Login"));
		ui.click(new JButtonLocator("REP"));
		ui.click(new JListLocator("Id: 0, Client: XYZ corp, createdBy: cso1, Status: APPROVED"));
		ui.click(new JButtonLocator("Send For Execution"));
		ui.click(new JListLocator("Id: 1, Client: XYZ corp, createdBy: cso1, Status: APPROVED"));
		ui.click(new JButtonLocator("Send For Execution"));
		ui.click(new JListLocator("Id: 2, Client: XYZ corp, createdBy: cso1, Status: APPROVED"));
		ui.click(new JButtonLocator("Send For Execution"));
		ui.click(new JListLocator("Id: 3, Client: XYZ corp, createdBy: cso1, Status: APPROVED"));
		ui.click(new JButtonLocator("Send For Execution"));
		ui.click(new JButtonLocator("Sign out"));
		ui.click(new LabeledTextLocator("Username:"));
		ui.enterText("pm");
		ui.click(new LabeledTextLocator("Password:"));
		ui.enterText("password");
		ui.click(new JButtonLocator("Login"));
		ui.click(new JButtonLocator("Crd"));
		ui.click(new JListLocator("Id: 0, Budget:0, Client: XYZ corp"));
		ui.click(new JButtonLocator("Tasks"));
		ui.click(new JButtonLocator("previous "));
		ui.click(new JListLocator("Id: 0, Budget:0, Client: XYZ corp"));
		ui.click(new JButtonLocator("Financial Request"));
		ui.click(new JButtonLocator("Create"));
		ui.click(new JListLocator("Id: 1, Budget:0, Client: XYZ corp"));
		ui.click(new JButtonLocator("Financial Request"));
		ui.click(new JButtonLocator("Create"));
		ui.click(new JListLocator("Id: 0, Budget:0, Client: XYZ corp"));
		ui.click(new JButtonLocator("recruitment Request"));
		ui.click(new JButtonLocator("previous "));
		ui.click(new JButtonLocator("previous "));
		ui.click(new JButtonLocator("Finance"));
		ui.click(new JButtonLocator("previous "));
		ui.click(new JButtonLocator("signout"));
		ui.click(new LabeledTextLocator("Username:"));
		ui.enterText("fm");
		ui.click(new LabeledTextLocator("Password:"));
		ui.enterText("password");
		ui.click(new JButtonLocator("Login"));
		ui.click(new JButtonLocator("Finance"));
		ui.click(new JListLocator("Id: 0, Department:"));
		ui.click(new JButtonLocator("Accept"));
	}

}